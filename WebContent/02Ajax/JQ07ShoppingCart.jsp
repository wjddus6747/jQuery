		...
		$('#goodsOptionId1').change(function() {
			...
			$.ajax({
				...
				success : function(d) {
					...
					//5.옵션을 처음 선택한다면 기존문자열은 지워주고 옵션을 추가한다.
					if ($('#goodsList').html() == "여기에 선택한 옵션이 리스팅됨") {
						$('#goodsList').empty();
					}

					//요청이 성공한 경우 옵션항목이 추가된다.
					$('#goodsList').append(d);
				},
				error : function(e) {
					alert("실패" + e.status);
		...
		<div class="row">
			...
			<div class="col-md-6">
				...
				<div class="row" id="priceDisplay"
					style="font-size: 1.5em; color: red;">총상품금액 : 70,000원</div>
			</div>
		</div>
	</div>
